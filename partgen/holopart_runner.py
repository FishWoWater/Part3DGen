...
class HoloPartRunner:
    def __init__(self, device: str = "cuda", 
        ...
        self.pipe: HoloPartPipeline = HoloPartPipeline.from_pretrained(holopart_weights_dir).to(device, dtype)
        self.device = device
    ...
    def prepare_data(self, data_path: str):
        if data_path.endswith(".glb"):
            ...
            batch_data = {
                ...
                "part_local_cond": torch.stack(part_local_cond_list, dim=0).to(self.device),
    ...
    def run_holopart(self, mesh_input: str, num_inference_steps: int = 50, guidance_scale: float = 3.5, seed: int = 2025):
        # only expose necessary parameters
        parts_data = self.prepare_data(mesh_input)
        return self._run(parts_data, self.batch_size, seed, num_inference_steps=num_inference_steps, guidance_scale=guidance_scale, device=self.device)
    ...
    def _run(
        ...
        for i, mesh_latent in enumerate(meshes_latent):
            ...
            if use_flash_decoder:
                ...
                geometric_func = lambda x: self.pipe.vae.decode(mesh_latent, sampled_points=x).sample
